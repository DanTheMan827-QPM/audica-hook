...
enum INSTRUCTION_TYPE {
    ...
    // 在对ADD进行修正时，采用了替换PC为Rr的方法，当Rd也为PC时，由于之前更改了Rr的值，可能会影响跳转后的正常功能;实际汇编中没有发现Rm也为PC的情况，故未做处理。
    ADD_ARM,
    // ADR Rd, <label>
    ADR1_ARM,
    // ADR Rd, <label>
...
static void relocateInstructionInThumb(
    ...
    while (1) {
        ...
        if ((orig_instructions[orig_pos] >> 11) >= 0x1D && (orig_instructions[orig_pos] >> 11) <= 0x1F) {
            ...
            offset = relocateInstructionInThumb32(
                pc, orig_instructions[orig_pos], orig_instructions[orig_pos + 1], &trampoline_instructions[trampoline_pos]
            );
            pc += sizeof(uint32_t);
...
static void relocateInstructionInArm(
    uint32_t target_addr,
...
void relocateInstruction(
    ...
    if ((target_addr & 1) == 1) {
        relocateInstructionInThumb(
            target_addr - 1, (uint16_t*) orig_instructions, length, (uint16_t*) trampoline_instructions, orig_boundaries, trampoline_boundaries, count
        );
    } else {
        relocateInstructionInArm(
            target_addr, (uint32_t*) orig_instructions, length, (uint32_t*) trampoline_instructions, orig_boundaries, trampoline_boundaries, count
        );
    }
}
#endif
